...
namespace STL {
	...
	template<class T,class Allocator=CMP::TCMemAllocator>
	class BinTree {
	public:
		//typedef base_node<T><T> base_node<T>;
	...
	private:
		void create_PRE(node*& root, const char* s, int& pos) {
			if (s[pos] == '#') {
				...
				//root = new node(s[pos++]);
				root = (node*)Allocator::Alloc(sizeof(node));
				new(root) node(s[pos++]);
				_size++;
		...
		void create_POSTIN(node*& root, const vector<T>& inorder, const vector<T>& postorder) {
			...
			size_t size = inorder.size();
			//root = new node(postorder[size - 1]);
			root = (node*)Allocator::Alloc(sizeof(node));
			new(root) node(postorder[size - 1]);
			_size++;
	...
	//	//STL::Tree<int> T(inorder, preorder, true);
	//	STL::BinTree<int> T(inorder, postorder, false);
	//	T.LevelTraverse();
	//}
