...
namespace STL {
	...
	template<class T, class compare = less<T>,class Allocator = CMP::TCMemAllocator>
	class LeftHeap :public SplayTree<T, T, KOD1<T>, compare,Allocator> {
	public:
	...
	protected:
		...
		size_t erase(const T& key)override {
			...
			base_node<T>* L = this->_root->left;
			base_node<T>* R = this->_root->right;

			//delete this->_root;
			Allocator::Free(this->root);
			this->_size--;
		...
		base_node<T>* merge_heap(LeftHeap& LH) {
			LH._root = nullptr; LH._size = 0;
			return this->_root = merge(this->_root, LH._root);
		}

		static base_node<T>* merge(base_node<T>* a, base_node<T>* b) {
			if (!a)return b;
			if (!b)return a;
