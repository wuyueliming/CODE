...
namespace xdh {
	...
	class AVLTree :public SplayTree<Key, T, KeyOfData, compare> {
	public:
		...
		virtual pair<Iterator, bool> insert(const T& data) {
			...
			while (pos) {
				if (pos->BF() < -1 || pos->BF() > 1) {
					base_node* parent = pos->parent; node* child = (node*)pos->taller_child();
					base_node*& ptr = SplayTree<Key, T, KeyOfData, compare>::ParentPtrRefFrom(pos);
