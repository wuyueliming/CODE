...
namespace xdh {
	...
	class RBTree :public SplayTree<Key, T, KeyOfData, compare> {
	public:
		...
		struct update_H {
			void operator()(const base_node* const pnode) {
	...
	protected:
		...
		void double_red(base_node* pos) {//pos:插入节点
			...
			if (!uncle || uncle->color == BLACK) {
				base_node*& ptr = SplayTree<Key, T, KeyOfData, compare>::ParentPtrRefFrom(grandparent);
		...
		void double_black(base_node* pos) {//pos:代替节点父亲
			...
			base_node*& ptr = SplayTree<Key, T, KeyOfData, compare>::ParentPtrRefFrom(parent);
			if (brother->color==BLACK) {//兄弟为黑
				...
				if (red_L || red_R) {//兄弟有红孩子->(BB-1)
					...
					base_node* nephew = red_L ? brother->left : brother->right;
					...
					if (parent->color == RED) {//(BB-2R)
						...
						base_node* grandparent = parent->parent;
				...
				base_node* nephew = brother->isleft() ? brother->left : brother->right;
		...
		bool dark_balanced(base_node* root) {
			node* pos = (node*)root;
