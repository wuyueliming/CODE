...
namespace xdh {
	...
	public:
		...
		BST_iterator(base_node* pnode, base_node* root) {
			_pnode = (node*)pnode;
			_root = (node*)root;
		}
	...
	public:
		typedef base_node<T> base_node;
		typedef BSTreeNode<T> node;//node内部存储指针都是base_node*，除了使用指针访问特殊成员或者new node时用node*
		//其余传地址、访问data或者左右指针都使用base_node*
	...
	public:
		...
		//返回插入后pos的位置
		base_node* insert(base_node* pos, const T& data) {
		...
		virtual pair<Iterator, bool> insert(const T& data) {
			...
			base_node* pre = p.second;

			base_node* newnode = new node(data, nullptr, nullptr, pre);
		...
		virtual Iterator lower_bound(const Key& key) {
			...
			base_node* ret = p.first;
			base_node* next_ret = _search_(ret->left, key).first;
			while (next_ret) {
				ret = next_ret;
		...
		virtual Iterator upper_bound(const Key& key) {
			...
			base_node* ret = ((node*)p.first)->succ();
			return Iterator(ret,this->_root);
		}
	...
	protected:
		...
		base_node*& ParentPtrRefFrom(base_node* ptr) {
			node* p = (node*)ptr;
		...
		base_node* CopyTree(base_node* parent, base_node* copyroot) {
		...
		pair<base_node*, base_node*> _remove_(node* del, node** return_add) {
			if (del->left && del->right) {
				...
				return remove(succ);
				//return pair<base_node*, base_node*>(del, del->parent);
			}
			else {
				node* replace = del->left ? (node*)del->left : (node*)del->right;
				pair<base_node*, base_node*> ret(replace, del->parent);
				if (!del->parent) {
					this->_root = replace;
				}
				else {
		...
		//返回实际删除后代替节点指针和父节点指针
		template<class node_type=node>
		pair<base_node*, base_node*> remove(node* del) {
			node* new_node;
			base_node* parent_ptr = ParentPtrRefFrom(del);
		...
		pair<base_node*, base_node*> _search_ (base_node* root, const Key& key) {
			KeyOfData KOD = KeyOfData();
			compare cmp = compare();

			base_node* pre = nullptr;
			base_node* cur = root;//如果出现bug，把这里的root改成this->_root就是原来的search了。
			while (cur) {
				if (cmp(key, KOD(cur->data))) {
					...
					return pair<base_node*, base_node*>(cur, pre);
				}
			}
			return pair<base_node*, base_node*>(nullptr, pre);
		}

		//返回查找节点指针和父节点指针
		pair<base_node*, base_node*> search(const Key& key) {
			return _search_(this->_root, key);
		}



		virtual bool check(base_node* root) {
			if (!root)return true;
