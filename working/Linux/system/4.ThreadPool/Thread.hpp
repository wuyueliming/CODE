...
namespace ThreadModule{
    ...
    class Thread
    {                                       //更有拓展性可以和pthread库一样使用void* (void*)类型的
    ...
    private:
        ...
        //如果func只是一个普通函数，想要传入参数可以在thread内加一个args存放，在routine内回调时传进去。但是要注意args和res指针的内存泄露问题。
        //void* args;
        void *res;
        func_t _func;//如果给线程一个包装的struct task，可以在task内部存放输入输出参数。
    };
